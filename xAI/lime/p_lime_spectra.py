...
f'{k_widths[k_n]}_kernel/{k_widths[k_n]}_k_outlier_exp_AE.csv', 'w', newline='\n')
...
for j, outlier in enumerate(outliers_to_exp):
    ...
    exp.save_to_file(
    file_path=f'{k_widths[k_n]}_kernel/{j}_outlier_k_{k_widths[k_n]}_exp_AE.html')
    ...
    exp_fig.savefig(f'{k_widths[k_n]}_kernel/{j}_outlier_k_{k_widths[k_n]}_exp_AE.png')
