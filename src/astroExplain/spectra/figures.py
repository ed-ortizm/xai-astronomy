...
def different_explanations(
    wave: np.array, explanations: list, figsize: tuple = None, labels=list[str]
) -> tuple:

    explanation_weights = []

    spectrum = TellMeWhy(wave=wave, explanation=explanations[0]).galaxy
    ...
    fig, axs = plt.subplots(
        nrows=1 + len(explanation_weights),
        ncols=1,
        sharex=True,
        tight_layout=True,
        figsize=figsize,
